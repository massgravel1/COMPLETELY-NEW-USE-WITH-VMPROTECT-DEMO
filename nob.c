...
#define LAZER_SOURCES  "SSSISANIGGER/*.cpp", "SSSISANIGGER/lazer/*.cpp", "SSSISANIGGER/lazer/features/*.cpp", \
                       "vendor/imgui/*.cpp", "vendor/imgui/lazer/*.cpp"

#define LEGACY_SOURCES "SSSISANIGGER/*.cpp", "SSSISANIGGER/legacy/*.cpp", "SSSISANIGGER/legacy/features/*.cpp", \
                       "vendor/imgui/*.cpp", "vendor/imgui/legacy/*.cpp"

#define DEMO_SOURCES "SSSISANIGGER/*.cpp", "SSSISANIGGER/lazer/ui.cpp", "SSSISANIGGER/demo/*.cpp", \
...
static bool build_lazer(Cmd *cmd)
{
    ...
    nob_log(INFO, "BUILD: SSSISANIGGER-lazer");
    cmd_append(cmd, cxx);
    ...
    cmd_append(cmd, "-MP");
    if (!*debug)
        cmd_append(cmd, "-link", "-DLL", "-OUT:SSSISANIGGER-lazer.dll", "-LTCG", "-MACHINE:x64", "vendor/minhook/minhook.x64.mt.lib");
    else
        cmd_append(cmd, "-link", "-DLL", "-OUT:SSSISANIGGER-lazer.dll", "-LTCG", "-DEBUG", "-MACHINE:x64", "vendor/minhook/minhook.x64.mtd.lib");
#else
    UNREACHABLE("SSSISANIGGER-lazer: Not Implemented");
#endif // _MSC_VER
    return cmd_run_sync_and_reset(cmd);
}
...
{
    build_injector(cmd, "SSSISANIGGER");
    nob_log(INFO, "BUILD: SSSISANIGGER");
    cmd_append(cmd, cxx);
    if (*git_commit_hash) cmd_append(cmd, temp_sprintf("-DGIT_COMMIT_HASH=%s", git_commit_hash));
    if (*console) cmd_append(cmd, "-DFR_LOG_TO_CONSOLE");
    if (*debug) cmd_append(cmd, "-DFR_DEBUG");
    ...
    if (!*debug)
        ...
        cmd_append(cmd, "-link", "-DLL", "-OUT:SSSISANIGGER.dll", "-DEBUG", "-MACHINE:x86");
#else
    UNREACHABLE("SSSISANIGGER: Not Implemented");
#endif // _MSC_VER
    return cmd_run_sync_and_reset(cmd);
}
...
{
    nob_log(INFO, "BUILD: SSSISANIGGER-ui-demo");
    cmd_append(cmd, cxx);
    if (*git_commit_hash) cmd_append(cmd, temp_sprintf("-DGIT_COMMIT_HASH=%s", git_commit_hash));
    if (*console) cmd_append(cmd, "-DFR_LOG_TO_CONSOLE");
    if (*debug) cmd_append(cmd, "-DFR_DEBUG");
    ...
    cmd_append(cmd, "-MP");
    if (!*debug)
        cmd_append(cmd, "-link", "-OUT:SSSISANIGGER-ui-demo.exe", "-LTCG", "-MACHINE:x64", "vendor/GLFW/glfw3.x64.mt.lib");
    else
        cmd_append(cmd, "-link", "-OUT:SSSISANIGGER-ui-demo.exe", "-DEBUG", "-MACHINE:x64", "vendor/GLFW/glfw3.x64.mt.lib");
#else
    UNREACHABLE("SSSISANIGGER-ui-demo: Not Implemented");
#endif // _MSC_VER
    return cmd_run_sync_and_reset(cmd);
}
...
int main(int argc, char **argv)
{
    ...
    lazer = flag_bool("lazer", false, "build SSSISANIGGER-lazer");
    legacy = flag_bool("legacy", false, "build SSSISANIGGER");
    demo = flag_bool("demo", false, "build SSSISANIGGER-ui-demo");
    ...
    if (*run && ok) {
        ...
        if (*legacy)
            run_executable("SSSISANIGGER");
        if (*demo)
            run_executable("SSSISANIGGER-ui-demo");
    }
